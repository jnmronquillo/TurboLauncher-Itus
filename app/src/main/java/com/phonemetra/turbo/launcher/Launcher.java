...
import org.thoughtcrime.securesms.authentication.utils.SharedPreferenceWrapper;
import ca.uwaterloo.crysp.itus.storage.BinLabel;
import ca.uwaterloo.crysp.itus.storage.DataStorage;

public class Launcher extends SecureActivity implements View.OnClickListener,
	...
	//itus
	private static final String TAG_ITUS_FRAGMENT = "itus_fragment";
	private ItusFragment mItusFragment;
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        //ArrayList<Integer> localScore = new ArrayList<Integer>();
        TouchEventLive touchEventLive = new TouchEventLive();
        final Touchalytics touchalytics = new Touchalytics(touchEventLive);
		...
		mItusFragment = (ItusFragment) fm.findFragmentByTag(TAG_ITUS_FRAGMENT);

		// If the Fragment is non-null, then it is currently being
		// retained across a configuration change.
		if (mItusFragment == null) {
			mItusFragment = new ItusFragment();
			fm.beginTransaction().add(mItusFragment, TAG_ITUS_FRAGMENT).commit();
		}
	}
	...
	private void dispatchOnLauncherTransitionEnd(View v, boolean animated,
		...
		//itus
		/*if (!mItusFragment.enoughData())
			Log.e("Log", "Training "+ DataStorage.binSize(BinLabel.BIN_TRAIN));
		else{
			Log.e("Log", "Classification"+DataStorage.binSize(BinLabel.BIN_RECENT));
			//ArrayList<Integer> scores = touchalytics.getPastScores();
			ArrayList<Integer> scores = mItusFragment.getPastScores();
			Log.e("Score size", Integer.toString(scores.size()));
			for (int score : scores) {
				Log.e("Log", "score: "+score);
				if (score < 0) {

					//Intent pin_lock = new Intent(getApplicationContext(), PinLock.class);
					//pin_lock.putExtra("category", "imm_trans");
					//startActivity(pin_lock);
					Toast.makeText(Launcher.this,"PinLock",Toast.LENGTH_SHORT).show();
					apw.comparePattern();
				}
			}
		}*/
